...
namespace Tests.Pointer
  ...
  /-- Create a pointer from a value and read it back. -/
  testcase testFromValue := do
    let m ← Pointer.fromValue .int64 (.int 42)
    assertEqual m.size 8 s!"invalid size: {m.size}"
    assertEqual (← m.read 0 .int64) (.int 42)

  /-- Create a pointer from a struct and read it back. -/
  testcase testFromStruct := do
    let tp := CType.struct #[.int8, .int8, .int8]
    let m ← Pointer.fromValue tp (.struct #[.int 42, .int 43, .int 44])
    assertEqual (← m.read 0 tp) (.struct #[.int 42, .int 43, .int 44])

  /-- Create a pointer from a struct with an array. -/
  testcase testFromStruct_array := do
    let tp := CType.struct #[.int8, .array .uint8 4]
    discard <| Pointer.fromValue tp (.struct #[.int 42, .array #[.nat 0, .nat 1, .nat 2, .nat 3]])

  /-- Read all integer types. -/
  testcase testRead_int8 := do
    let m ← Pointer.fromByteArray $ .mk (#[(0xFF : UInt8)] * 8)
    let v ← m.read 0 .int8
    assertEqual v (.int (-1))
  ...
  testcase testRead_float := do
    let m ← Pointer.fromByteArray $ .mk #[0xdb, 0x0f, 0x49, 0x40]
    let v ← m.read 0 .float
    assertEqual s!"{repr v}" s!"{repr (LeanValue.float 3.141593)}"

  testcase testRead_double := do
    let m ← Pointer.fromByteArray $ .mk #[0x18, 0x2d, 0x44, 0x54, 0xfb, 0x21, 0x09, 0x40]
    let v ← m.read 0 .double
    assertEqual s!"{repr v}" s!"{repr (LeanValue.float 3.141593)}"

  testcase testRead_longdouble := do
    let m ← Pointer.fromByteArray $ .mk #[0x00, 0xc0, 0x68, 0x21, 0xa2, 0xda, 0x0f, 0xc9,
  ...
  testcase testRead_complex_float := do
    let m ← Pointer.fromByteArray $ .mk #[0xdb, 0x0f, 0x49, 0x40, 0xdb, 0x0f, 0x49, 0xc0]
  ...
  testcase testRead_struct := do
    let m ← Pointer.fromByteArray $ .mk #[0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
  ...
  testcase testDereferenceEmpty := do
    let m ← Pointer.fromByteArray $ .mk (#[(0x00 : UInt8)] * 8)
